...
{
#ifdef _WIFI_TEST_
	uint8_t infor_type = WIFI_INFOR;
	uint8_t data_type = ALL;
	char sbuf[SBUF_SZ];
	uint8_t i = 0;
	uint8_t index;
#ifdef _TCP_DEBUG_
...
#endif
	// Start
	snprintf(json_str, STORE_LINE_SZ, "{\"Imei\":\"%s\",\"FlagConfig\":%u,\"Index\":%u,",
			 g_imei, data_type, infor_type);
...
void json_cfg_ethernet_create(void)
{
	...
	uint8_t infor_type = ETHERNET_INFOR;
	uint8_t data_type = ALL;
	char sbuf[SBUF_SZ];
#ifdef _TCP_DEBUG_
